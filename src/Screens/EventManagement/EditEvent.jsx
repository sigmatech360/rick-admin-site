...
export const EditEvent = () => {
    ...
    fetch(`${apiUrl}/api/admin/event/${id}`, {
      ...
      .then((data) => {
        ...
        if (data.data.description) {
          setEditorValue(
            RichTextEditor.createValueFromString(data.data.description, "html")
          );
        }
      })
      .catch((error) => {
  ...
  function formatToReadableDate(isoDateStr) {
    ...
    return formatted.replace(/\d+/, `${day}${suffix(day)}`);
  }
  ...
  const handleChange = (event) => {
    ...
    //   newValue = convertTo12Hour(value);
    // }
    // else if (name === "date") {
  ...
  const handleSubmit = (event) => {
    ...
    fetch(`${apiUrl}/api/admin/event/update/${id}`, {
      ...
      .then((data) => {
        document.querySelector(".loaderBox").classList.add("d-none");
        console.log(data);
        if (data?.status == true) setShowModal(true);
  ...
  const handleRemove = (imageid) => {
    const indeximage = formData?.product_images.findIndex(
      (item) => item.id === imageid
    );
    if (indeximage !== -1) {
      setFormData((prevFormData) => {
        const productimage = [
          ...prevFormData?.product_images.slice(0, indeximage),
          ...prevFormData?.product_images.slice(indeximage + 1),
        ];
        return {
          ...prevFormData,
                    ...
                    <div className="row">
                      ...
                      <div className="col-md-6 mb-4">
                        <CustomInput
                          ...
                          value={
                            formData?.start_time
                              ? convertTo24Hour(formData.start_time)
                              : ""
                          }
                          onChange={handleChange}
                        />
                      ...
                      <div className="col-md-6 mb-4">
                        <CustomInput
                          ...
                          value={
                            formData?.end_time
                              ? convertTo24Hour(formData.end_time)
                              : ""
                          }
                          onChange={handleChange}
                        />
