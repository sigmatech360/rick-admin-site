...
export const VolunteerManagement = () => {
  ...
  const handleAssign = (id, status) => {
    ...
    const assignusers =
      status == "Assigned"
        ? `${apiUrl}/api/admin/event/un-assign-user`
        : `${apiUrl}/api/admin/event/assign-user`;
    fetch(assignusers, {
      method: "POST",
      headers: {
        Accept: "application/json",
  ...
  const handleTop = async (id, isTop) => {
    if (!isTop) {
      ...
      try {
        ...
        const assignusers = `${apiUrl}/api/admin/top-volunteer/${id}`;
    ...
    <>
      <DashboardLayout>
        <div className="container-fluid">
                      ...
                      <tbody>
                        {currentItems?.map((item, index) => (
                          <tr key={index}>
                            ...
                            <td
                              className={
                                item?.is_active == 1 ? "greenColor" : "redColor"
                              }
                            >
                            ...
                            <td>
                              <Dropdown className="tableDropdown">
                                ...
                                >
                                  ...
                                  <Link
                                    to={`/volunteer-management/volunteer-details/${item?.id}`}
                                  ...
                                  {/* <Link to={`/volunteer-management/volunteer-details/${item?.id}`} className="tableAction"><FontAwesomeIcon icon={faTrash} className="tableActionIcon" />Delete</Link> */}
                                  {item?.is_top !== undefined && (
                                    <button
                                      type="button"
                                      className="tableAction"
                                      onClick={() =>
                                        Volusteeractiveinactive(item?.id)
                                      }
                                    >
                                      {item?.is_active == 1 ? (
                                        <FontAwesomeIcon
                                  ...
                                  {item?.status && (
                                    <button
                                      ...
                                      className="tableAction"
                                      onClick={() =>
                                        handleAssign(item?.id, item?.status)
                                      }
                                    >
          ...
          />
          <CustomModal
            show={topstatusmodalstatus}
            close={() => {
              setTopstatusmodalstatus(false);
            }}
            success
            heading={topstatusmodal}
          />
        </div>
      </DashboardLayout>
    </>
